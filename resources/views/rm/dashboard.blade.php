                                    ...
                                    <div class="col-md-10">
                                      <button type="button" name="submit" id="submitbtn" class="btn btn-primary"
                                        ...
                                        <button type="button" class="btn btn-secondary"
                                            onclick="downloadrmPatientExcel();">Patient Report</button>
                                    </div>
                                </div>
                            </form>
...
<script>
    $(document).ready(function () {
        ...
        function educator()
        {
            $.post("{{ route('common.getEducatorbyrmzoneandRm') }}", {
        ...
        let patientTable = $('#patientTable').DataTable({
            ...
            ajax: {
                url: "{{ route('common.getrmPatientTable') }}",
                type: "POST",
                data: function (d) {
                    return $.extend({}, d, {
                        _token: $('input[name="_token"]').val(),
        ...
        function loadCamps() {
            $.post("{{ route('common.getCampbyrmeducators') }}", {
        ...
        $('#campId').on('change', function () {
            $.post("{{ route('common.getDoctorsByrmEdu') }}", {
                educatorId: $('#educator').val(),
                _token: $('input[name="_token"]').val()
            }, function (data) {
                $('#doctor').html(data);
        ...
        educator(); // Initial call to load educators
    });

    window.downloadrmPatientExcel = function () {
        ...
        var url = "{{ route('rm.patients.export') }}"
