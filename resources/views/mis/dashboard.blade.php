...
<script>
    $(document).ready(function () {
        ...
        let patientTable = $('#patientTable').DataTable({
            ...
            ajax: {
                url: "{{ route('common.getmisPatientTable') }}",
                type: "POST",
                data: function (d) {
                    return $.extend({}, d, {
        ...
        $('#zone').on('change', function () {
            $.post("{{ route('common.misgetrmsbyzone') }}", {
                zone: $(this).val(),
                _token: $('input[name="_token"]').val()
            }, function (data) {
                $('#rm').html(data);
        ...
        $('#rm').on('change', function () {
            $.post("{{ route('common.misgetEducatorbyzoneandRm') }}", {
                rm: $(this).val(),
                zone: $('#zone').val(),
                _token: $('input[name="_token"]').val()
            }, function (data) {
    ...
    window.downloadpmPatientExcel = function () {
        ...
        var url = "{{ route('mis.patients.export') }}"
    ...
    var url = "{{ route('misdaily.report.export') }}"
        + "?fromDate=" + encodeURIComponent(fromDate)
        + "&toDate=" + encodeURIComponent(toDate);
