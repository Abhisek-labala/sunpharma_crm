            ...
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        ...
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('dashboard.digitaleducator') }}">Dashboard</a></li>
                        ...
                        <div class="col-md-12">
                            <h4>Current Status: 
                                ...
                                @if($todayAttendance && !$todayAttendance->out_time)
                                    <button class="btn btn-danger btn-sm ms-3" id="markOutBtn">Mark Out Time</button>
                                @endif
                            </h4>
...
$(document).ready(function() {
    ...
    function updateAttendance(lat, lng, address, state, ip) {
        ...
        $.ajax({
            url: "{{ route('digitaleducator.attendance.updateLocation') }}",
            type: "POST",
            data: data,
    ...
    $('#markOutBtn').click(function() {
        Swal.fire({
            title: 'Are you sure?',
            text: "You are about to mark your out time!",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('digitaleducator.attendance.markOut') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
