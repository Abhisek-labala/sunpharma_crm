    ...
    $(document).ready(function () {
        ...
        let patientTable = $('#patientTable').DataTable({
           processing: true,
            ...
            ajax: {
                url: "{{ route('common.getdigitaleduPatientTable') }}",
                type: "POST",
                data: function (d) {
                    return $.extend({}, d, {
        ...
        function loadCamps() {
            $.post("{{ route('common.getdigiCampbyeducator') }}", {
        ...
        function loadZones() {
            $.post("{{ route('common.getdigiZones') }}", {
                _token: $('input[name="_token"]').val()
            }, function (data) {
                $('#zone').html(data);
        ...
        $('#zone').on('change', function () {
            $.post("{{ route('common.getrmsbydigizone') }}", {
                zone: $(this).val(),
                _token: $('input[name="_token"]').val()
            }, function (data) {
                $('#rm').html(data);
        ...
        $('#rm').on('change', function () {
            $.post("{{ route('common.getEducatorbydigizoneandRm') }}", {
                rm: $(this).val(),
                zone: $('#zone').val(),
                _token: $('input[name="_token"]').val()
            }, function (data) {
